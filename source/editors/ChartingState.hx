...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function updateGrid():Void
	{
		...
		if (_song.notes.length > 0 && _song.notes[curSection] != null && _song.notes[curSection].changeBPM && _song.notes[curSection].bpm > 0)
		{
		...
		// NEXT SECTION
		if(curSection < _song.notes.length-1 && curSection > 0) {
	...
	{
		PlayState.SONG = Song.loadFromJson(song.toLowerCase() + CoolUtil.difficultyStuff[PlayState.storyDifficulty][1], song.toLowerCase());
		MusicBeatState.resetState();
	}
