...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		rankText.setFormat(Paths.font("vcr.ttf"), 48);
		rankText.size = scoreText.size;
		rankText.screenCenter(X);

		var ui_tex = Paths.getSparrowAtlas('campaign_menu_UI_assets');
		var bgYellow:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 386, FlxColor.fromRGB(230, 199, 150));
		bgSprite = new FlxSprite(0, 56);
		bgSprite.antialiasing = ClientPrefs.globalAntialiasing;

		var cat = new FlxSprite().loadGraphic(Paths.image('kitty-face'));
		cat.screenCenter(X);
		cat.y += 182;
		...
		{
			if (!WeekData.weeksLoaded.get(WeekData.weeksList[i]).hideStoryMode)
			{
		...
		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 90, 0, "", 32);
	...
	override function update(elapsed:Float)
	{
		...
		scoreText.text = "WEEK BURRITO:" + lerpScore;
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		sprDifficultyGroup.forEach(function(spr:FlxSprite) {
			...
			if(curDifficulty == spr.ID) {
				...
				FlxTween.tween(spr, {y: leftArrow.y + 5, alpha: 1}, 0.07);
			}
		});
